...
match l with
|nil => l
|h::l1 => if(oddb h) then h :: oddmembers l1 else oddmembers l1 end.
...
match s with
|nil => 0
|h::t => if(v =? h) then 1 + (count v t) else count v t end.
...
Fixpoint exist (v:nat) (s:bag) : bool :=
  match s with
  | nil => false
  | h :: t => if( v =? h) then true else exist v t end.
...
(* FILL IN HERE   : infinity des éléments *)
